...
preinstall_wordpress(){
	if [ ! -f ${VH_DOC_ROOT}/wp-config.php ] && [ -f ${VH_DOC_ROOT}/wp-config-sample.php ]; then
		...
		NEWDBPWD="define('DB_PASSWORD', '${MYSQL_PASSWORD}');"
    linechange 'DB_PASSWORD' ${VH_DOC_ROOT}/wp-config.php "${NEWDBPWD}"
		NEWDBPWD="define('DB_USER', '${MYSQL_USER}');"
		linechange 'DB_USER' ${VH_DOC_ROOT}/wp-config.php "${NEWDBPWD}"
		NEWDBPWD="define('DB_NAME', '${MYSQL_DATABASE}');"
...
cd ${VH_DOC_ROOT} && wget --no-check-certificate http://wordpress.org/latest.tar.gz
tar -xzvf latest.tar.gz  --strip-components=1 >/dev/null 2>&1
rm latest.tar.gz
